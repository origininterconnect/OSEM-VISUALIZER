...
const Connection: React.FC<ConnectionProps> = ({
    ...
    const [selectedBitsValue, setSelectedBitsValue] = useState(10);
    ...
    const connectToDevice = () => {
        ...
        wsRef.current.onopen = function () {
            ...
            commands.forEach((cmd) => wsRef.current?.send(JSON.stringify(cmd)));
        };
        Connection(true);
    ...
    const parseEEGData = (buffer: Uint8Array) => {
        ...
        for (let blockLocation = 0; blockLocation < buffer.length; blockLocation += blockSize) {
            ...
            // Extract data for each channel (assuming 3 bytes per channel)
            for (let channel = 0; channel < 8; channel++) {
                const offset = 8 + channel * 3;

                // Convert 3 bytes to a 24-bit signed integer
                let sample = (block[offset] << 16) | (block[offset + 1] << 8) | block[offset + 2];
                // Process multiple channels
                channelData[channel] = Filter[channel].process(sample);
            }
                                    ...
                                    <div className={`relative w-full flex flex-col ${!isDisplay ? "" : "items-start"} text-sm`}>
                                        ...
                                        <div className="relative w-[28rem] flex items-center rounded-lg py-2 border border-gray-300 dark:border-gray-600 mb-4">
                                            ...
                                            <input
                                                ...
                                                style={{
                                                    background: `linear-gradient(to right, rgb(101, 136, 205) ${((Zoom - 1) / 1874) * 100}%, rgb(165, 165, 165) ${((Zoom - 1) / 1874) * 100}%)`,
                                                }}
